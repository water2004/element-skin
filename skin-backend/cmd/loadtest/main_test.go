...
func TestParseFlagsAppliesEveryCommandLineOverrideExactly(t *testing.T) {
	...
	os.Args = []string{
		...
		"-path=/v2/admin/users?limit=7",
		"-method=patch",
		`-body={"name":"Updated"}`,
		"-content-type=application/v2/users/merge-patch+json",
		"-concurrency=2,4,8",
		"-duration=3s",
		"-timeout=750ms",
		"-fail-threshold=2.5",
	...
	want := options{
		...
		method:          "patch",
		body:            `{"name":"Updated"}`,
		contentType:     "application/v2/users/merge-patch+json",
...
func TestBuildURL(t *testing.T) {
	got, err := buildURL("http://127.0.0.1:8000/api", "/v2/public/settings")
	if err != nil {
		t.Fatal(err)
	}
	if got != "http://127.0.0.1:8000/api/v2/public/settings" {
		t.Fatalf("unexpected URL: %s", got)
	}
	got, err = buildURL("http://127.0.0.1:8000/api", "/v2/admin/users?limit=20&q=Load")
	if err != nil {
		t.Fatal(err)
	}
	if got != "http://127.0.0.1:8000/api/v2/admin/users?limit=20&q=Load" {
		t.Fatalf("query string should stay as query, got: %s", got)
	}
	got, err = buildURL("http://ignored", "https://example.com/v2/users/me")
	if err != nil {
		t.Fatal(err)
	}
	if got != "https://example.com/v2/users/me" {
		t.Fatalf("absolute URL should pass through: %s", got)
	}
	if _, err := buildURL("127.0.0.1:8000", "/v2/users/me"); err == nil {
...
func TestLoginSendsExactCredentialsAndReturnsCookieHeader(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		calls.Add(1)
		if req.Method != http.MethodPost || req.URL.Path != "/api/v2/auth/login" {
			t.Fatalf("login request=%s %s; want POST /api/v2/auth/login", req.Method, req.URL.Path)
		}
	...
	cookies, err := login(server.Client(), server.URL+"/api", "/v2/auth/login", "load@example.com", "Secret123")
...
func TestDoRequestForwardsExactRequestAndClassifiesResponse(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if req.Method != http.MethodPatch ||
			req.Header.Get("Content-Type") != "application/v2/users/merge-patch+json" ||
