...
func TestMicrosoftHTTPClientExchangeCodeRequestShape(t *testing.T) {
	...
	client := microsoft.MicrosoftHTTPClient{
		...
		ClientID:     "client_id",
		ClientSecret: "secret",
		RedirectURI:  "https://skin.example/v2/imports/microsoft/callback",
	}
	...
	if gotForm["client_id"] != "client_id" || gotForm["client_secret"] != "secret" || gotForm["code"] != "auth_code" ||
		gotForm["redirect_uri"] != "https://skin.example/v2/imports/microsoft/callback" || gotForm["grant_type"] != "authorization_code" {
