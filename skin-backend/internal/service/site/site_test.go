...
func TestSiteAuthAccountAndSessionExactState(t *testing.T) {
	...
	if err := db.AddRefreshToken(ctx, "change_password_refresh", userID, database.NowMS()+60_000, database.NowMS()); err != nil {
		t.Fatal(err)
	}
	if err := site.ChangePassword(ctx, userID, "Password123", "NewPassword123"); err != nil {
		t.Fatal(err)
	}
	changed, err := db.GetUserByID(ctx, userID)
	if err != nil {
		t.Fatal(err)
	}
	...
	if refresh, err := db.GetRefreshToken(ctx, "change_password_refresh"); err != nil || refresh != nil {
...
func TestSiteProfilesTexturesAndLibraryExactState(t *testing.T) {
	...
	if err := db.AddTextureToLibrary(ctx, user.ID, "site_skin", "skin", "Site Skin", true, "slim"); err != nil {
		t.Fatal(err)
	}
	if err := db.AddTextureToLibrary(ctx, user.ID, "site_cape", "cape", "Site Cape", false, "default"); err != nil {
		t.Fatal(err)
	}
	...
	withTextures, err := db.GetProfileByID(ctx, profileID)
	if err != nil {
		t.Fatal(err)
	}
	...
	afterModel, err := db.GetProfileByID(ctx, profileID)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := db.AdminUpdateTexturePublic(ctx, "site_skin", true); err != nil {
		t.Fatal(err)
	}
	...
	cleared, err := db.GetProfileByID(ctx, profileID)
	if err != nil {
		t.Fatal(err)
	}
	...
	if cape, err := db.GetTextureInfo(ctx, user.ID, "site_cape", "cape"); err != nil || cape != nil {
	...
	if deleted, err := db.GetProfileByID(ctx, profileID); err != nil || deleted != nil {
...
func TestSiteVerificationAndResetPasswordExactState(t *testing.T) {
	...
	code, expiresAt, ok, err := db.GetVerificationCode(ctx, "new-register@test.com", "register")
	if err != nil {
		t.Fatal(err)
	}
	...
	resetCode, _, ok, err := db.GetVerificationCode(ctx, "reset-site@test.com", "reset")
	if err != nil || !ok {
	...
	if err := db.AddRefreshToken(ctx, "reset_refresh", user.ID, database.NowMS()+60_000, database.NowMS()); err != nil {
		t.Fatal(err)
	}
	...
	if _, _, ok, err := db.GetVerificationCode(ctx, "reset-site@test.com", "reset"); err != nil || ok {
		t.Fatalf("ResetPassword should consume reset verification code: ok=%v err=%v", ok, err)
	}
	if refresh, err := db.GetRefreshToken(ctx, "reset_refresh"); err != nil || refresh != nil {
