...
func TestAdminProfilesTexturesInvitesHTTP(t *testing.T) {
	...
	forbiddenProfiles := doJSON(t, h, "GET", "/v2/admin/profiles", nil, userCookie)
	if forbiddenProfiles.Code != 403 {
		t.Fatalf("non-admin profiles list should be 403, got %d", forbiddenProfiles.Code)
	}
	profiles := doJSON(t, h, "GET", "/v2/admin/profiles?q=AdminList1", nil, adminCookie)
	...
	patchProfile := doJSON(t, h, "PATCH", "/v2/admin/profiles/"+profile.ID, map[string]any{"name": "AdminRenamed"}, adminCookie)
	if patchProfile.Code != http.StatusNoContent || patchProfile.Body.Len() != 0 {
	...
	duplicateProfile := doJSON(t, h, "PATCH", "/v2/admin/profiles/"+profile.ID, map[string]any{"name": otherProfile.Name}, adminCookie)
	...
	forbiddenPatchProfile := doJSON(t, h, "PATCH", "/v2/admin/profiles/"+profile.ID, map[string]any{"name": "Nope"}, userCookie)
	if forbiddenPatchProfile.Code != 403 {
		t.Fatalf("non-admin profile patch should be 403, got %d", forbiddenPatchProfile.Code)
	}
	missingProfileDelete := doJSON(t, h, "DELETE", "/v2/admin/profiles/missing-profile", nil, adminCookie)
	...
	forbiddenClearSkin := doJSON(t, h, "PATCH", "/v2/admin/profiles/"+profile.ID+"/skin", map[string]any{"hash": nil}, userCookie)
	if forbiddenClearSkin.Code != 403 {
		t.Fatalf("non-admin clear skin should be 403, got %d", forbiddenClearSkin.Code)
	}
	clearSkin := doJSON(t, h, "PATCH", "/v2/admin/profiles/"+profile.ID+"/skin", map[string]any{"hash": nil}, adminCookie)
	if clearSkin.Code != http.StatusNoContent || clearSkin.Body.Len() != 0 {
	...
	clearCape := doJSON(t, h, "PATCH", "/v2/admin/profiles/"+profile.ID+"/cape", map[string]any{"hash": nil}, adminCookie)
	if clearCape.Code != http.StatusNoContent || clearCape.Body.Len() != 0 {
	...
	missingClearCape := doJSON(t, h, "PATCH", "/v2/admin/profiles/missing-profile/cape", map[string]any{"hash": nil}, adminCookie)
	...
	forbiddenTextures := doJSON(t, h, "GET", "/v2/admin/textures", nil, userCookie)
	if forbiddenTextures.Code != 403 {
		t.Fatalf("non-admin textures list should be 403, got %d", forbiddenTextures.Code)
	}
	textures := doJSON(t, h, "GET", "/v2/admin/textures?q=AdminTexture", nil, adminCookie)
	...
	patchTex := doJSON(t, h, "PATCH", "/v2/admin/textures/adm_hash", map[string]any{"type": "skin", "is_public": 0, "note": "AdminRenamedTexture"}, adminCookie)
	if patchTex.Code != http.StatusNoContent || patchTex.Body.Len() != 0 {
	...
	forbiddenPatchTexture := doJSON(t, h, "PATCH", "/v2/admin/textures/adm_hash", map[string]any{"type": "skin", "is_public": 1}, userCookie)
	if forbiddenPatchTexture.Code != 403 {
		t.Fatalf("non-admin texture patch should be 403, got %d", forbiddenPatchTexture.Code)
	}
	forbiddenDeleteTexture := doJSON(t, h, "DELETE", "/v2/admin/textures/adm_hash?user_id="+user.ID+"&type=skin", nil, userCookie)
	if forbiddenDeleteTexture.Code != 403 {
		t.Fatalf("non-admin texture delete should be 403, got %d", forbiddenDeleteTexture.Code)
	}
	delTex := doJSON(t, h, "DELETE", "/v2/admin/textures/adm_hash?user_id="+user.ID+"&type=skin", nil, adminCookie)
	if delTex.Code != http.StatusNoContent || delTex.Body.Len() != 0 {
	...
	invites := doJSON(t, h, "GET", "/v2/admin/invites", nil, adminCookie)
	if invites.Code != 200 {
...
func TestAdminUserControlsHTTP(t *testing.T) {
	...
	search := doJSON(t, h, "GET", "/v2/admin/users?q=NormalControls", nil, adminCookie)
	if search.Code != 200 {
	...
	detail := doJSON(t, h, "GET", "/v2/admin/users/"+user.ID, nil, adminCookie)
	if detail.Code != 200 {
	...
	missingDetail := doJSON(t, h, "GET", "/v2/admin/users/missing-user", nil, adminCookie)
	...
	invalidCursor := doJSON(t, h, "GET", "/v2/admin/users?cursor=garbage!!", nil, adminCookie)
	if invalidCursor.Code != 400 {
		t.Fatalf("invalid admin user cursor should be 400, got %d", invalidCursor.Code)
	}

	selfGrant := doJSON(t, h, "POST", "/v2/admin/users/"+admin.ID+"/protected-subject/transfer", nil, adminCookie)
	...
	granted := doJSON(t, h, "PUT", "/v2/admin/users/"+user.ID+"/roles/admin", nil, adminCookie)
	if granted.Code != http.StatusNoContent || granted.Body.Len() != 0 {
	...
	revoked := doJSON(t, h, "DELETE", "/v2/admin/users/"+user.ID+"/roles/admin", nil, adminCookie)
	...
	ban := doJSON(t, h, "POST", "/v2/admin/users/"+user.ID+"/ban", map[string]any{"banned_until": bannedUntil, "reason": "integration ban"}, adminCookie)
	if ban.Code != 200 {
	...
	unban := doJSON(t, h, "POST", "/v2/admin/users/"+user.ID+"/unban", nil, adminCookie)
	if unban.Code != http.StatusNoContent || unban.Body.Len() != 0 {
	...
	reset := doJSON(t, h, "POST", "/v2/admin/users/password/reset", map[string]any{"user_id": user.ID, "new_password": "NewStr0ngPass!"}, adminCookie)
	...
	missingReset := doJSON(t, h, "POST", "/v2/admin/users/password/reset", map[string]any{"user_id": "missing", "new_password": "x"}, adminCookie)
	...
	del := doJSON(t, h, "DELETE", "/v2/admin/users/"+user.ID, nil, adminCookie)
	...
	missingDelete := doJSON(t, h, "DELETE", "/v2/admin/users/"+user.ID, nil, adminCookie)
...
func TestAdminTextureValidationEdges(t *testing.T) {
	...
	invalidPublic := doJSON(t, h, "PATCH", "/v2/admin/textures/somehash", map[string]any{"is_public": 2}, adminCookie)
	...
	missingPublic := doJSON(t, h, "PATCH", "/v2/admin/textures/nonexistent-hash", map[string]any{"is_public": 0}, adminCookie)
	...
	missingNote := doJSON(t, h, "PATCH", "/v2/admin/textures/nonexistent-hash", map[string]any{"note": "missing"}, adminCookie)
	...
	invalidModel := doJSON(t, h, "PATCH", "/v2/admin/textures/somehash", map[string]any{"model": "invalid"}, adminCookie)
	if invalidModel.Code != 400 {
		t.Fatalf("invalid model should be 400, got %d body=%s", invalidModel.Code, invalidModel.Body.String())
	}
	missingModel := doJSON(t, h, "PATCH", "/v2/admin/textures/nonexistent-hash", map[string]any{"model": "slim"}, adminCookie)
	...
	missingUser := doJSON(t, h, "DELETE", "/v2/admin/textures/somehash?type=skin", nil, adminCookie)
	...
	force := doJSON(t, h, "DELETE", "/v2/admin/textures/force_hash?type=skin&force=true", nil, adminCookie)
	if force.Code != http.StatusNoContent || force.Body.Len() != 0 {
