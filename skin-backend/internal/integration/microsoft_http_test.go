...
func TestMicrosoftImportProfileTokenSemantics(t *testing.T) {
	...
	resp := doJSON(t, h, "POST", "/v2/imports/microsoft/profile/import", map[string]any{
	...
	replay := doJSON(t, h, "POST", "/v2/imports/microsoft/profile/import", map[string]any{"ms_token": importToken}, cookie)
	if replay.Code != 400 {
	...
	other := doJSON(t, h, "POST", "/v2/imports/microsoft/profile/import", map[string]any{"ms_token": otherUserToken}, cookie)
	if other.Code != 403 {
	...
	wrongKind := doJSON(t, h, "POST", "/v2/imports/microsoft/profile/import", map[string]any{"ms_token": wrongKindToken}, cookie)
	if wrongKind.Code != 400 {
		t.Fatalf("wrong kind token should be 400, got %d body=%s", wrongKind.Code, wrongKind.Body.String())
	}

	missing := doJSON(t, h, "POST", "/v2/imports/microsoft/profile/import", map[string]any{"ms_token": "does-not-exist"}, cookie)
	if missing.Code != 400 {
	...
	conflict := doJSON(t, h, "POST", "/v2/imports/microsoft/profile/import", map[string]any{"ms_token": conflictToken}, cookie)
	...
	dedup := doJSON(t, h, "POST", "/v2/imports/microsoft/profile/import", map[string]any{"ms_token": nameDedupToken}, cookie)
	if dedup.Code != 200 {
...
func TestMicrosoftAuthURLAndGetProfileTokenSemantics(t *testing.T) {
	...
	authURL := doJSON(t, h, "GET", "/v2/imports/microsoft/auth-url", nil, cookie)
	if authURL.Code != 200 {
	...
	getProfileOther := doJSON(t, h, "POST", "/v2/imports/microsoft/profile", map[string]any{"ms_token": profileToken}, otherCookie)
	...
	getProfile := doJSON(t, h, "POST", "/v2/imports/microsoft/profile", map[string]any{"ms_token": profileToken}, cookie)
	...
	replay := doJSON(t, h, "POST", "/v2/imports/microsoft/profile", map[string]any{"ms_token": profileToken}, cookie)
	if replay.Code != 400 {
		t.Fatalf("profile token should be one-shot, got %d body=%s", replay.Code, replay.Body.String())
	}
	importResp := doJSON(t, h, "POST", "/v2/imports/microsoft/profile/import", map[string]any{"ms_token": importToken}, cookie)
