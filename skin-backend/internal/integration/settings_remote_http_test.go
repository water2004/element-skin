...
func TestSettingsGroupsAndRemoteYggImportHTTP(t *testing.T) {
	...
	defaults := doJSON(t, h, "GET", "/v2/admin/settings/site", nil, adminCookie)
	...
	securityDefaults := parseJSON(t, doJSON(t, h, "GET", "/v2/admin/settings/security", nil, adminCookie))
	...
	emailDefaults := parseJSON(t, doJSON(t, h, "GET", "/v2/admin/settings/email", nil, adminCookie))
	...
	invalidGroup := doJSON(t, h, "POST", "/v2/admin/settings/nonsense", map[string]any{"x": 1}, adminCookie)
	if invalidGroup.Code != 400 {
		t.Fatalf("invalid settings group should be 400, got %d", invalidGroup.Code)
	}
	invalid := doJSON(t, h, "POST", "/v2/admin/settings/site", map[string]any{"profile_uuid_mode": "bogus"}, adminCookie)
	if invalid.Code != 400 {
		t.Fatalf("invalid profile_uuid_mode should be 400, got %d", invalid.Code)
	}
	afterInvalid := parseJSON(t, doJSON(t, h, "GET", "/v2/admin/settings/site", nil, adminCookie))
	if afterInvalid["profile_uuid_mode"] != "random" {
		t.Fatalf("invalid profile_uuid_mode should not persist: %#v", afterInvalid)
	}
	save := doJSON(t, h, "POST", "/v2/admin/settings/site", map[string]any{"site_name": "Public Name", "allow_register": false, "max_texture_size": 2048}, adminCookie)
	if save.Code != http.StatusNoContent || save.Body.Len() != 0 {
		t.Fatalf("save settings status=%d body=%s", save.Code, save.Body.String())
	}
	securitySave := doJSON(t, h, "POST", "/v2/admin/settings/security", map[string]any{"rate_limit_enabled": true, "rate_limit_auth_attempts": 10}, adminCookie)
	if securitySave.Code != http.StatusNoContent || securitySave.Body.Len() != 0 {
		t.Fatalf("security save status=%d body=%s", securitySave.Code, securitySave.Body.String())
	}
	securitySaved := parseJSON(t, doJSON(t, h, "GET", "/v2/admin/settings/security", nil, adminCookie))
	...
	emailSave := doJSON(t, h, "POST", "/v2/admin/settings/email", map[string]any{"smtp_host": "mail.example.com", "smtp_password": ""}, adminCookie)
	if emailSave.Code != http.StatusNoContent || emailSave.Body.Len() != 0 {
	...
	emailSaved := parseJSON(t, doJSON(t, h, "GET", "/v2/admin/settings/email", nil, adminCookie))
	if emailSaved["smtp_host"] != "mail.example.com" {
		t.Fatalf("email settings did not roundtrip smtp_host: %#v", emailSaved)
	}
	public := doJSON(t, h, "GET", "/v2/public/settings", nil)
	...
	for _, body := range badFallbacks {
		resp := doJSON(t, h, "POST", "/v2/admin/settings/fallback", body, adminCookie)
		if resp.Code != 400 {
			t.Fatalf("bad fallback settings should be 400, got %d body=%s for %#v", resp.Code, resp.Body.String(), body)
		}
	}
	fallbackSave := doJSON(t, h, "POST", "/v2/admin/settings/fallback", map[string]any{
	...
	if fallbackSave.Code != http.StatusNoContent || fallbackSave.Body.Len() != 0 {
		t.Fatalf("fallback save status=%d body=%s", fallbackSave.Code, fallbackSave.Body.String())
	}
	fallbackGroup := doJSON(t, h, "GET", "/v2/admin/settings/fallback", nil, adminCookie)
	...
	publicAfterFallback := parseJSON(t, doJSON(t, h, "GET", "/v2/public/settings", nil))
	...
	addWL := doJSON(t, h, "POST", "/v2/admin/official-whitelist", map[string]any{"username": "Whitelisted", "endpoint_id": endpointID}, adminCookie)
	if addWL.Code != http.StatusCreated {
		t.Fatalf("add whitelist status=%d body=%s", addWL.Code, addWL.Body.String())
	}
	listWL := doJSON(t, h, "GET", "/v2/admin/official-whitelist?endpoint_id="+strconv.Itoa(endpointID), nil, adminCookie)
	...
	removeWL := doJSON(t, h, "DELETE", "/v2/admin/official-whitelist/Whitelisted?endpoint_id="+strconv.Itoa(endpointID), nil, adminCookie)
	if removeWL.Code != http.StatusNoContent || removeWL.Body.Len() != 0 {
	...
	homepageEmpty := doJSON(t, h, "GET", "/v2/public/homepage-media", nil)
	...
	homepageUpload := doMultipart(t, h, "POST", "/v2/admin/homepage-media/image", nil, "file", "banner.png", pngTexture(t, 64, 64), adminCookie)
	if homepageUpload.Code != http.StatusCreated {
	...
	homepageList := doJSON(t, h, "GET", "/v2/public/homepage-media", nil)
	...
	homepageDelete := doJSON(t, h, "DELETE", "/v2/admin/homepage-media/"+mediaID, nil, adminCookie)
	if homepageDelete.Code != http.StatusNoContent || homepageDelete.Body.Len() != 0 {
	...
	notList := doJSON(t, h, "POST", "/v2/imports/remote-ygg/profiles/import-batch", map[string]any{"profiles": "bad"}, userCookie)
	if notList.Code != 400 {
		t.Fatalf("non-list profiles should be 400, got %d", notList.Code)
	}
	emptyList := doJSON(t, h, "POST", "/v2/imports/remote-ygg/profiles/import-batch", map[string]any{"profiles": []any{}}, userCookie)
	if emptyList.Code != 400 {
		t.Fatalf("empty profiles should be 400, got %d", emptyList.Code)
	}
	imported := doJSON(t, h, "POST", "/v2/imports/remote-ygg/profiles/import-batch", map[string]any{"profiles": []map[string]string{
