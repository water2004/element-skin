...
func TestRedisBackedPublicSettingsAndHomepageMediaHTTP(t *testing.T) {
	...
	first := doJSON(t, h, "GET", "/v2/public/settings", nil)
	...
	cached := doJSON(t, h, "GET", "/v2/public/settings", nil)
	...
	refreshed := doJSON(t, h, "GET", "/v2/public/settings", nil)
	...
	homepageMedia := doJSON(t, h, "GET", "/v2/public/homepage-media", nil)
...
func TestAdminSettingsInvalidatePublicCacheAndApplySecurityImmediately(t *testing.T) {
	...
	saveSite := doJSON(t, h, "POST", "/v2/admin/settings/site", map[string]any{"site_name": "Admin Saved Site"}, adminCookie)
	if saveSite.Code != http.StatusNoContent || saveSite.Body.Len() != 0 {
	...
	afterSite := doJSON(t, h, "GET", "/v2/public/settings", nil)
	...
	saveFallback := doJSON(t, h, "POST", "/v2/admin/settings/fallback", map[string]any{
	...
	if saveFallback.Code != http.StatusNoContent || saveFallback.Body.Len() != 0 {
		t.Fatalf("save fallback settings status=%d body=%s", saveFallback.Code, saveFallback.Body.String())
	}
	afterFallback := parseJSON(t, doJSON(t, h, "GET", "/v2/public/settings", nil))
	...
	saveSecurity := doJSON(t, h, "POST", "/v2/admin/settings/security", map[string]any{
		"rate_limit_enabled":       true,
		"rate_limit_auth_attempts": 1,
		"rate_limit_auth_window":   1,
	}, adminCookie)
	if saveSecurity.Code != http.StatusNoContent || saveSecurity.Body.Len() != 0 {
		t.Fatalf("save security settings status=%d body=%s", saveSecurity.Code, saveSecurity.Body.String())
	}
	firstLogin := doJSONFromIP(t, h, "POST", "/v2/auth/login", map[string]any{"email": "missing@test.com", "password": "bad"}, "203.0.113.77:10000")
	if firstLogin.Code != 401 {
		t.Fatalf("first login should reach auth path, got %d %s", firstLogin.Code, firstLogin.Body.String())
	}
	limited := doJSONFromIP(t, h, "POST", "/v2/auth/login", map[string]any{"email": "missing@test.com", "password": "bad"}, "203.0.113.77:10000")
	if limited.Code != http.StatusTooManyRequests {
		t.Fatalf("security settings should apply immediately to rate limiter, got %d %s", limited.Code, limited.Body.String())
	}

	saveAuth := doJSON(t, h, "POST", "/v2/admin/settings/auth", map[string]any{"jwt_expire_days": 2}, adminCookie)
	if saveAuth.Code != http.StatusNoContent || saveAuth.Body.Len() != 0 {
		t.Fatalf("save auth settings status=%d body=%s", saveAuth.Code, saveAuth.Body.String())
	}
	login := doJSON(t, h, "POST", "/v2/auth/login", map[string]any{"email": admin.Email, "password": "Password123"})
	if login.Code != 200 {
...
func TestRedisBackedRateLimitAndVerificationHTTP(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		resp := doJSONFromIP(t, h, "POST", "/v2/auth/login", map[string]any{"email": "missing@test.com", "password": "bad"}, "198.51.100.10:10000")
		if resp.Code != 401 {
			t.Fatalf("login attempt %d should reach auth path, got %d %s", i+1, resp.Code, resp.Body.String())
		}
	}
	limited := doJSONFromIP(t, h, "POST", "/v2/auth/login", map[string]any{"email": "missing@test.com", "password": "bad"}, "198.51.100.10:10000")
	...
	send := doJSON(t, h, "POST", "/v2/auth/verification-code", map[string]any{"email": "redis-register@test.com", "type": "register"})
	if send.Code != 200 {
...
func TestRedisBackedAuthCacheAndInvalidationHTTP(t *testing.T) {
	...
	users := doJSON(t, h, "GET", "/v2/admin/users", nil, cookie)
	if users.Code != 200 {
	...
	revoked := doJSON(t, h, "GET", "/v2/admin/users", nil, cookie)
	if revoked.Code != 403 {
	...
	demoted := doJSON(t, h, "GET", "/v2/admin/users", nil, cookie)
	if demoted.Code != 403 {
