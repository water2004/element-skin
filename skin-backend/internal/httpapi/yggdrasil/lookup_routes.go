...
func (h Handler) HasJoined(w http.ResponseWriter, req *http.Request) {
	...
	if status == 204 {
		resp, err := (fallbacksvc.Fallback{DB: h.db}).HasJoined(req.Context(), username, serverID, req.URL.Query().Get("ip"))
		if err != nil {
			util.Error(w, err)
			return
		}
...
func (h Handler) Profile(w http.ResponseWriter, req *http.Request) {
	...
	if status == 204 {
		resp, err := (fallbacksvc.Fallback{DB: h.db}).GetProfile(req.Context(), req.PathValue("uuid"), unsigned)
		if err != nil {
			util.Error(w, err)
			return
		}
...
func (h Handler) LookupName(w http.ResponseWriter, req *http.Request) {
	...
	if status == 204 {
		...
		if strings.HasPrefix(req.URL.Path, "/api/minecraft/profile/lookup/name/") || strings.HasPrefix(req.URL.Path, "/minecraft/profile/lookup/name/") {
			resp, err = (fallbacksvc.Fallback{DB: h.db}).ServicesLookup(req.Context(), playerName)
		} else {
			resp, err = (fallbacksvc.Fallback{DB: h.db}).GetProfileByName(req.Context(), playerName)
		}
		if err != nil {
			util.Error(w, err)
			return
...
func (h Handler) LookupNames(w http.ResponseWriter, req *http.Request) {
	...
	if len(missing) > 0 {
		fallbackProfiles, err := (fallbacksvc.Fallback{DB: h.db}).BulkLookup(req.Context(), missing)
		if err != nil {
			util.Error(w, err)
			return
		}
