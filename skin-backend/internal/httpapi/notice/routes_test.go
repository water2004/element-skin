...
func TestNoticeRoutesListDetailReadDismissExactFlow(t *testing.T) {
	...
	h.List(rec, userRequest(http.MethodGet, "/v2/notifications?limit=1&include_read=false&type=announcement", user.ID, false))
	...
	req := userRequest(http.MethodGet, "/v2/notifications/"+first.ID, user.ID, false)
	...
	req = userRequest(http.MethodPost, "/v2/notifications/"+second.ID+"/read", user.ID, false)
	...
	req = userRequest(http.MethodPost, "/v2/notifications/"+third.ID+"/dismiss", user.ID, false)
	...
	rec = httptest.NewRecorder()
	h.List(rec, userRequest(http.MethodGet, "/v2/notifications?include_read=true", user.ID, false))
...
func TestNoticeRoutesErrorsAndAuthWrapperExactly(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	h.Auth(h.List)(rec, httptest.NewRequest(http.MethodGet, "/v2/notifications?limit=1", nil))
	...
	req := userRequest(http.MethodGet, "/v2/notifications/missing", user.ID, false)
	...
	req = httptest.NewRequest(http.MethodPost, "/v2/notifications/missing/dismiss", nil)
	req.SetPathValue("id", "missing")
	rec = httptest.NewRecorder()
	...
	req = httptest.NewRequest(http.MethodPost, "/v2/notifications/missing/read", nil)
	req.SetPathValue("id", "missing")
	rec = httptest.NewRecorder()
	...
	req = userRequest(http.MethodPost, "/v2/notifications/missing/read", user.ID, false)
	req.SetPathValue("id", "missing")
	rec = httptest.NewRecorder()
	h.MarkRead(rec, req)
	if rec.Code != http.StatusNotFound || rec.Body.String() != "{\"error\":{\"object\":\"notice\",\"operation\":\"resolve\",\"reason\":\"not_found\"}}\n" {
	...
	req = userRequest(http.MethodPost, "/v2/notifications/missing/dismiss", user.ID, false)
	req.SetPathValue("id", "missing")
	rec = httptest.NewRecorder()
	h.Dismiss(rec, req)
	if rec.Code != http.StatusNotFound || rec.Body.String() != "{\"error\":{\"object\":\"notice\",\"operation\":\"resolve\",\"reason\":\"not_found\"}}\n" {
