...
> Element Skin 集成说明：本文只描述 Microsoft access token 到 Minecraft profile 的底层交换协议。站点不直接暴露 Microsoft 登录/导入 API。管理员应配置 `adapter=microsoft` 的普通 OIDC 身份提供方，用户通过统一外部身份流程绑定账号；本站正版角色绑定再按需调用本文的 Xbox / XSTS / Minecraft 交换链路。Microsoft refresh token 由统一身份凭据模块加密保存并按需刷新，不由定时任务刷新。
>
