...
明确事件的凭证失效规则如下；这些规则由对应业务 service 在事件发生时立即执行，不等待定时任务：

| 事件 | grant | access token | refresh token | authorization code |
| --- | --- | --- | --- | --- |
| 用户撤销单个授权 | 标记为 `revoked`，保留 30 天 | 删除该 grant 的全部 token | 标记为已撤销 | 删除该 grant 的全部 code |
| 管理员驳回或停用应用 | 该应用全部 active grant 标记为 `revoked` | 删除该应用全部 token | 该应用全部标记为已撤销 | 删除该应用全部 code |
| 开发者将 active 应用重新提交审核 | 与停用应用相同 | 与停用应用相同 | 与停用应用相同 | 与停用应用相同 |
| 轮换机密应用密钥 | 保持原状态 | 删除该应用全部 token | 该应用全部标记为已撤销 | 删除该应用全部 code |
| 修改回调地址或客户端类型 | 保持原状态 | 删除该应用全部 token | 该应用全部标记为已撤销 | 删除该应用全部 code |
| 应用删除某项申请权限 | 仅撤销包含已删除权限的 grant | 删除该应用已有 token，避免继续携带旧权限 | 仅撤销受影响 grant 的 refresh token | 仅删除受影响 grant 的 code |
| 管理员修改 `client:{client_id}` 权限覆盖 | 保持原状态 | 删除该应用全部 token | 保持原状态 | 保持原状态 |
| 删除应用 | 随应用物理删除 | 删除该应用全部 token | 随应用物理删除 | 随应用物理删除 |
| 删除用户 | 删除该用户授权的 grant 及其应用 | 删除该用户及其应用相关 token | 随 grant 或应用物理删除 | 随 grant 或应用物理删除 |

应用密钥轮换、回调地址变更和客户端类型变更不改变用户已经表达的授权意愿，因此不得把 grant 标记为 `revoked`；但变更前签发的任何凭证都不得继续使用。仅修改应用名称、说明或网站地址时，不得撤销 grant 或凭证。应用权限收窄时，仍满足新权限上限的 grant 及 refresh token 必须保留。单独调用 token revocation endpoint 只撤销指定 token，不等价于撤销整个 grant。
...
权限变动通知也是 API 行为的一部分。用户角色授权、角色撤销、单项权限覆盖新增、单项权限覆盖移除、受保护权限主体转移，均必须由系统维护 actor 创建 `type = system`、`audience = targeted` 的通知并投递给对应用户。若同一次权限变动导致 OAuth grant 自动撤销或 OAuth 应用自动停用，服务端还必须分别向授权用户或应用创建者发送系统通知。系统通知 `created_by` 必须为空，通知有效期为 30 天。
