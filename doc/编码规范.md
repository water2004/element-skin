...
- 必须维持代码库整洁。同一个功能只能有一个清晰实现，不允许并存多个冗余实现。
- 优先复用现有抽象。前端优先使用已有 UI 组件、API 层、存储层；后端优先使用已有 `httpapi`、`service`、`database` 分层。
- 不允许为了所谓兼容性长期保留旧路径、旧接口、旧组件或旧逻辑形成代码债。
- 兼容性代码仅允许出现在数据库迁移和数据修复路径中，并且必须有明确边界。
- 不合适的老接口和老实现应重构或删除。除数据库数据迁移外，网站功能不承担长期兼容旧实现的要求。
...
- 前端页面入口、导航项、操作入口应基于细粒度权限展示，不使用 `user`、`admin`、`super_admin` 等粗粒度身份判断能力。
- 前端权限判断只用于展示和交互引导，真正的访问控制必须由后端 service 层完成。
- 具有部分管理权限的普通用户应能看到对应功能入口；没有相关权限的页面入口不应显示。
- 新增页面或导航项时，应明确列出所依赖的权限，并同步更新 API 或权限文档。
...
- 数据库表必须结构化建模。除仅用于保存不可变原始载荷、不可修改快照或外部协议原文的字段外，不允许把业务数据塞进 JSON 字符串。
- 可查询、可筛选、可更新、可约束、可迁移的业务信息必须拆成明确列或关联表。
- 不允许用 `TEXT`/`JSON`/`JSONB` 字段规避表结构设计、权限控制、索引设计或数据迁移。
- 如确需保存原始 JSON 载荷，必须同时保存结构化字段作为业务读取来源，原始载荷只能作为审计、排错或不可变快照。
...
- 后端功能授权必须使用细粒度权限模型，不以 `user`、`admin`、`super_admin` 等粗粒度身份作为最终授权依据。
- 超级管理员只是拥有完整权限集合，不允许在业务代码里绕过权限模型做特判。
- 系统维护任务使用已有系统维护 actor 执行，不新增临时维护 actor，也不为绕过权限检查创建 wrapper。
- 账号封禁的站点语义是禁止加入服务器，应落实为对应 join/server 能力不可用，不扩大为无关能力限制。
- OAuth access token 属于短期访问凭证，应存放在 Redis 或等价短期 token store 中；refresh token、授权记录、客户端等长期状态才进入数据库。
...
- 生产配置必须显式提供，不为密钥、数据库、Redis、站点 URL 等关键配置保留危险默认值。
- 配置文件和环境变量的合并规则必须清晰；环境变量覆盖配置文件时，应以启动时可审计的方式写入或更新配置。
- 没有配置文件但环境变量完整时，生产环境应能启动；配置文件和环境变量都不完整时应明确失败。
- CORS 必须读取配置，不在代码中硬编码通配来源。
- 允许凭据的站点请求不得使用 `*` 作为 CORS 来源；第三方 OAuth 后端应用通常不依赖浏览器 CORS 调用受保护资源。
...
- Python SDK 位于 `python-sdk/`，公开 API 应保持稳定、类型清晰、异常语义明确。
- SDK 文档放在 `python-sdk/doc/`，面向中文开发者编写。
- SDK demo 可以提交脚本和无密钥配置模板，不提交真实 `client_secret`、access token、refresh token、设备码中间输出或本地回调临时文件。
- demo 的本地配置、token 缓存和授权输出必须由 `.gitignore` 忽略。
- demo 应覆盖代表管理员的 Client Credentials 场景，以及代表用户授权的 Authorization Code 或 Device Code 场景。
- 示例代码不得默认连接生产站点执行破坏性操作；涉及新增、删除、撤销等动作时必须在代码和文档中明确。
...
- 不把 OAuth access token、SDK demo token、本地 client secret 或授权输出提交进仓库。
- 不用粗粒度身份字段替代细粒度权限检查。
- 不在后端 handler 中堆积复杂业务规则。
- 不把数据库错误原样暴露给用户。
- 不在首页玻璃按钮外层加入会破坏 fixed 顶层和 backdrop blur 的布局容器。
- 不为同一功能保留多个实现入口、重复组件、重复工具函数或重复 API wrapper。
- 不保留“兼容旧行为”的屎山代码；旧行为不合适时应重构调用方或删除旧接口。
- 不在非数据库迁移代码中加入长期兼容分支。
- 不把应当结构化的业务数据以 JSON 字符串形式存入数据库。
