...
>
> v4.0.0 的自动数据库升级只支持正式发布的 **v3.0.0、v3.0.1、v3.0.2**。v2.x 站点必须先升级到 v3.0.2 并确认运行正常，再升级到 v4.0.0。
>
...
本版本以 v3.0.2 为功能比较基线，自动迁移入口只面向 v3.0.0 至 v3.0.2 的正式数据库结构。开发期间的中间结构不属于公开兼容面，也不保留对应迁移路径。
...
1. 确认当前站点运行正式发布的 v3.0.0、v3.0.1 或 v3.0.2。
2. 如果当前仍是 v2.x，先按照 v3.0.2 的升级说明完成 v2.x 到 v3.0.2 的迁移，并验证站点可以正常启动、登录和访问数据；不要跳过 v3 直接升级 v4。
