...
从 v3.0.0 升级时不需要执行额外数据库迁移，也不需要修改配置。使用 Authorization Code Flow 的第三方应用需要升级 Python SDK，或在 token 请求中补充与授权请求完全一致的 `redirect_uri`。直接调用管理员材质删除接口的程序需要读取 `ok` 字段。
